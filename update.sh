...
main() {
  for repo in fullstack-agent ai-memory-vault backtalk barehands ai-visualizer; do
    ...
    if [ ! -d "$HOME_DIR/$repo/.git" ]; then
      ...
      if [ "$repo" = "backtalk" ] && [ -f "$HOME_DIR/fullstack-agent/provider_bridge/patch_backtalk.py" ]; then
        python3 "$HOME_DIR/fullstack-agent/provider_bridge/patch_backtalk.py" "$HOME_DIR/backtalk"
      fi
      echo "   wired to updates. everything is current."
      continue
    fi
    echo "== $repo"
    # The API bridge owns these Backtalk source files. Restore upstream
    # before pulling, then regenerate the overlay after the update.
    if [ "$repo" = "backtalk" ] && \
       grep -q "OpenAI-compatible brain for Backtalk" "$HOME_DIR/backtalk/backtalk/brain.py" 2>/dev/null; then
      git -C "$HOME_DIR/backtalk" checkout -q -- \
        backtalk/brain.py backtalk/main.py backtalk/config.py pyproject.toml
      REPATCH_BACKTALK=1
    fi
