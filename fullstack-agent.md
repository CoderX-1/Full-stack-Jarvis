...
You are the user's local coding agent, and you are about to assemble a complete one: memory, voice, face, and hands. This file is the conductor. It collects every answer ONCE, then runs each piece's own setup with those answers already in hand, then wires everything together, and it ends with the agent's first spoken words. This edition does not require Claude: its brain is OpenAI, Gemini through Google's OpenAI-compatible endpoint, or another OpenAI-compatible API.

Ground rules, binding for the whole run:

- **Plain English.** Assume the person started using an AI coding agent yesterday. Every technical thing gets a one-line explanation before it gets a name.
...
- **An `AGENT.md`, `AGENTS.md`, `GEMINI.md`, or `CLAUDE.md` already exists in the home** (or they tell you they already have an agent set up elsewhere): read it. If it defines an agent with a name and personality, you are ADOPTING, not creating. Say something like "found [name], keeping them exactly as they are," and skip every identity question later.
...
**Scope of that answer, precisely.** The fresh-or-existing question is about prior INSTALLS of these pieces (a voice system, a visualizer, vault software). "Brand new" binds exactly that and nothing more; it does not mean the person has no history with AI coding tools.

Three scanning rules that hold for the whole run:

1. **Old provider memory is optional migration material.** Claude Code memory at `~/.claude/projects/*/memory/`, Gemini context files, and prior agent markdown may be copied into the new vault only after listing what was found and asking which items to adopt. Migration copies; it never deletes originals.
...
**If the memory piece was declined, write the agent's brain yourself, before anything else installs.** Create a short canonical `AGENT.md` in the HOME folder carrying the identity from Phase 2: the agent's name, role, personality, welcome line, and one line saying this folder is where the agent lives. Then copy the same content to `AGENTS.md`, `GEMINI.md`, and `CLAUDE.md` so every supported client sees the same identity. Keep it minimal; it grows when they're ready. No piece of this stack ever runs brainless.
...
1. **ai-memory-vault** first (it creates the vault and writes the person's `CLAUDE.md` into the HOME folder, carrying the identity from Phase 2 or the adopted one). After its wizard finishes, copy that identity/instruction content to `AGENT.md`, `AGENTS.md`, and `GEMINI.md`; these are compatibility mirrors, not independent personalities. **Run its Part 1 whenever Obsidian is missing: check for the app, offer to install it (that wizard carries the exact per-platform commands), and never skip it, soften it, or call it optional. A fullstack setup that ends without Obsidian installed is incomplete, whatever else works.**
2. **backtalk** second. Immediately after cloning and before dependency installation, run `python fullstack-agent/provider_bridge/patch_backtalk.py backtalk` from HOME (use the working Python executable found during setup). This replaces Backtalk's Claude SDK brain with the provider-neutral API bridge and removes that SDK dependency. Then follow its installer for the local speech environment and models. Ignore Claude-specific account/login steps in Backtalk's upstream wizard; the chosen API environment variable is the brain authentication.
...
- `backtalk/backtalk.json`: `provider` = `"openai"`, `"gemini"`, or `"compatible"`; `model` and `deep_model` = the models chosen for quick voice replies and deeper work; `base_url` = the custom endpoint only for a compatible service (otherwise leave it empty). Never put the API key here.
...
Last wire: **make the agent the mechanic.** Append a short section to the canonical agent instructions in the home folder (for adopted instructions, show the person the section and ask before adding it), then refresh all provider mirrors:
...
**If yes:** install from https://github.com/jaredrhod/ai-marketing-skills, following that repo's own setup, placing the files in their vault and supported provider instruction/skill locations. Then give them the series that makes those files worth having: **The AI Marketing Machine**, https://youtube.com/playlist?list=PLdNHCeiXnovo . Frame the pair honestly: the files teach their AGENT the playbook, the series teaches THEM what to point it at.
...
1. **`Chat with <name>`** opens a typed API-backed session in the home folder, terminal only. macOS/Linux runs `python3 "<home folder>/fullstack-agent/agent.py" --cwd "<home folder>" --provider "<provider>" --model "<model>"`; Windows uses the equivalent `python` command. Add `--base-url "<url>"` for a custom endpoint. The launcher's environment must contain the provider key, but the key itself must never be written into the launcher.
...
A double-clicked shortcut launches with a bare system PATH where Python or `uv` may be absent, so a launcher without the export fails silently. (Windows `.bat` files inherit the user's PATH and do not need it.)
...
- **Starting a new window reloads the durable memory:** API chat history is intentionally temporary, while the vault and provider instruction files persist. Say the folder rule once, plainly: the agent only wakes up as itself when the runner opens in its home folder, which is exactly what the shortcuts do. Opened anywhere else, it lacks that identity and memory wiring.
...
- **Updating, and tell them this plainly:** on macOS, double-click `Update <name>` for the newest version of everything; it shows what changed, then applies it, and it never touches their files. The updater automatically reapplies the Backtalk API bridge. On Windows, say "update everything and tell me what changed" in any chat session — the agent pulls each repo and then reruns `provider_bridge/patch_backtalk.py` so voice compatibility remains installed.
